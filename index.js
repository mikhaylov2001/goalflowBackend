...
app.get("/api/goals", async (req, res) => {
  try {
    const [goalsRes, tasksRes, microsRes] = await Promise.all([
      ...
      pool.query("SELECT * FROM tasks ORDER BY sort_order NULLS LAST, created_at"),
      pool.query("SELECT * FROM microtasks ORDER BY sort_order NULLS LAST, created_at"),
    ]);

    const microsByTask = {};
    for (const m of microsRes.rows) {
      if (!microsByTask[m.task_id]) microsByTask[m.task_id] = [];
      microsByTask[m.task_id].push({
        id: m.id,
        title: m.title,
        done: m.done,
      });
...
app.post("/api/goals", async (req, res) => {
  try {
    ...
    await pool.query(
      `
      INSERT INTO goals (
        id, title, description, category, priority, deadline, reward,
        notif_enabled, notif_freq, notif_time, notif_day
      )
...
app.post("/api/tasks/:id/toggle", async (req, res) => {
  ...
  try {
    ...
    const taskRes = await client.query("SELECT id, done, goal_id FROM tasks WHERE id = $1", [
      req.params.id,
    ]);
    ...
    const goalDone =
      allTasksRes.rows.length > 0 && allTasksRes.rows.every((row) => row.done === true);

    await client.query("UPDATE goals SET done = $2 WHERE id = $1", [task.goal_id, goalDone]);
...
app.post("/api/microtasks/:id/toggle", async (req, res) => {
  ...
  try {
    ...
    const microRes = await client.query(
      "SELECT id, task_id, done FROM microtasks WHERE id = $1",
      [req.params.id]
    );
    ...
    const siblingsRes = await client.query("SELECT done FROM microtasks WHERE task_id = $1", [
      micro.task_id,
    ]);
    const taskDone =
    ...
    await client.query("COMMIT");
    res.json({ done: newDone, taskDone });
  } catch (err) {
    await client.query("ROLLBACK");
    console.error(err);
    res.status(500).json({ error: "Failed to toggle microtask" });
  } finally {
    client.release();
  }
});
...
app.get("/api/calendar", async (req, res) => {
  try {
    const result = await pool.query(
      "SELECT * FROM calendar_tasks ORDER BY task_date ASC, priority ASC, created_at ASC"
    );
    res.json(result.rows.map(mapCalendarTask));
  } catch (err) {
    console.error(err);
...
app.get("/api/habits", async (req, res) => {
  try {
    ...
    for (const row of logsRes.rows) {
      ...
      logsByHabit[row.habit_id][row.log_date] = true;
    }
...
app.post("/api/habits", async (req, res) => {
  try {
    ...
    await pool.query("INSERT INTO habits (id, title, emoji, color) VALUES ($1,$2,$3,$4)", [
      hid,
      title,
      emoji || "",
      color || "#6366F1",
...
app.post("/api/habits/:id/toggle", async (req, res) => {
  try {
    ...
    if (existing.rows.length > 0) {
      await pool.query("DELETE FROM habit_logs WHERE habit_id=$1 AND log_date=$2", [
        req.params.id,
        date,
      ]);
    } else {
      await pool.query("INSERT INTO habit_logs (habit_id, log_date) VALUES ($1,$2)", [
        req.params.id,
        date,
      ]);
    }

    const logsRes = await pool.query("SELECT log_date FROM habit_logs WHERE habit_id=$1", [
      req.params.id,
    ]);

    const streak = calcStreak(logsRes.rows.map((row) => row.log_date));

    await pool.query("UPDATE habits SET streak=$2 WHERE id=$1", [req.params.id, streak]);

    res.json({
      streak,
      toggled: existing.rows.length === 0,
    });
  } catch (err) {
    console.error(err);
...
app.post("/api/wishes", async (req, res) => {
  try {
    ...
    await pool.query("INSERT INTO wishes (id, title, note, emoji) VALUES ($1,$2,$3,$4)", [
      wid,
      title,
      note || "",
      emoji || "",
...
app.put("/api/wishes/:id", async (req, res) => {
  try {
    ...
    await pool.query("UPDATE wishes SET title=$2, note=$3, emoji=$4, done=$5 WHERE id=$1", [
      req.params.id,
      title,
      note || "",
      emoji || "",
...
app.listen(PORT, () => {
  console.log(`🚀 GoalFlow API on port ${PORT}`);
});
